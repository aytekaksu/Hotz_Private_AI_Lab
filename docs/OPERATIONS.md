...
- `bun run scripts/install.ts <domain> [acme-email]` — end-to-end VPS installer (written in Bun) that handles apt packages, Docker, repo cloning (requires `GITHUB_USER` + `GITHUB_PAT`), and calls the bootstrap CLI. When running on a fresh VPS, install Bun first (or use the provided one-liner) and then execute this command from the repo root.
