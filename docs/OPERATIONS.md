...
- `scripts/install.sh <domain> [acme-email]` — end-to-end VPS installer that handles apt packages, Bun, Docker, cloning the repo (requires `GITHUB_USER` + `GITHUB_PAT` env vars), and invokes the bootstrap CLI. Fetch it via the GitHub Raw API using the same credentials you use for `git clone`.
