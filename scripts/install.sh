...
Environment variables:
  ...
  ACME_EMAIL    Fallback email for TLS provisioning (optional if passed as arg)
...
if [[ -z "$ACME_EMAIL_INPUT" ]]; then
  echo "ACME email required (pass as second argument or set ACME_EMAIL env)." >&2
  exit 1
fi
...
ACME_EMAIL="$ACME_EMAIL_INPUT" bun run bootstrap "$DOMAIN" "$ACME_EMAIL_INPUT"
